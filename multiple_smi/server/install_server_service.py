...
parser = argparse.ArgumentParser(
    description="Install server smi as a service to enable it at boot",
...
parser.add_argument(
    ...
    default=None,
    help=(
        "path to server_smi binary file. If not set, will take the output of 'which"
        " server_smi'"
    ),
)
parser.add_argument(
    ...
    help="path to systemd service files",
)
parser.add_argument(
    "--port",
    "-p",
...
parser.add_argument(
    ...
    default=1,
    type=int,
    help="Time rate at which it will listen to sockets",
)
parser.add_argument(
    "--store-history",
    "-s",
    action="store_true",
    help="will store gpu usage in a csv file",
)
parser.add_argument(
    "--history-rate",
    "--hr",
    ...
    type=int,
    metavar="N",
    help="Will store history usage every N loop",
)
parser.add_argument(
    "--history-path",
    "--hp",
    default="/etc/.server_smi",
...
parser.add_argument(
    ...
    "-u",
    action="store_true",
    help="If selected, will deisable the service and delete the service file",
)
...
def uninstall(args):
    try:
        ...
        if os.path.isfile(args.service_file_path):
            ...
            raise Exception(
                "'server_smi.service' file not found and was not removed,\n"
                "although it is still available for systemctl.\n"
                "Check specified systemd folder argument"
            )
    except CalledProcessError:
        raise Exception(
            "Service file not available for systemctl,\n"
...
def install(args):
    if args.server_smi_path is None:
        args.server_smi_path = shutil.which("sever_smi")
    if args.server_smi_path is None:
        bin_path = Path(sys.argv[0]).parent.resolve()
        server_smi_path = bin_path / "server_smi"
        if server_smi_path.is_file() and os.access(server_smi_path, os.X_OK):
            args.server_smi_path = str(server_smi_path)
        else:
            raise Exception(
                "server_smi script not found, probably because you didn't install"
                " server_smi in sudo.\nYou might want to specify the litteral path"
                " to user-installed server_smi"
            )

    if args.store_history:
        history_param_string = f"-s --hr {args.history_rate} --hp {args.history_path}"
    else:
        history_param_string = ""

    service_final_string = service_string.format(
        server_smi_path=args.server_smi_path,
    ...
    while answer is None:
        raw_answer = input(
            'Run it now? (will launch "systemctl start server_smi.service") [Y/n]'
        )

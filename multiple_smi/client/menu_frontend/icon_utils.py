...
def get_default_colors(ratio=1.2):
    ...
    default_colors = np.stack(
        (
            cspace_convert(JCh_1, "JCh", "sRGB255"),
            cspace_convert(JCh_2, "JCh", "sRGB255"),
        ),
        axis=1,
    )
...
def draw_icon(machine):
    ...
    if summary["nGPUs"] > 2:
        WIDTH = (
            11 * summary["nGPUs"]
        )  # if more than 1 GPU on a machine, each column is 11px wide (and not 22px)
    surface = cairo.ImageSurface(cairo.FORMAT_ARGB32, WIDTH, HEIGHT)
    ctx = cairo.Context(surface)
    if summary["nGPUs"] > 0:
        ctx.scale(
            WIDTH / summary["nGPUs"], HEIGHT
        )  # Normalizing the canvas coordinates go from (0,0) to (nGPUs,1)

    for i in range(summary["nGPUs"]):
        gpu = summary["GPUs"][i]
        percentage1, percentage2 = (
            gpu["utilization"] / 100,
            gpu["used_mem"] / gpu["memory"],
        )
        ...
        ctx.rectangle(
            i + 0.5, 1 - percentage2, 0.5, percentage2
